...
from ignite.metrics import Average, Accuracy ##, ConfusionMatrix, Precision, Recall
...
class SentenceTransformer(nn.Sequential):
    ...
    def fit(self,
        ...
        for loss: writer (SummaryWriter): tensorboard summary writer
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                for train_idx in range(num_train_objectives):
                    ...
                    ## loss
                    writer.add_scalar(
                        "Loss/train per batch (noisy)", train_loss.compute().item(), global_step
                    )
            ...
            ## loss
            writer.add_scalar(
                "loss/train per Epoch (smooth)",
                train_metrics["train_loss"],
                epoch,
            )
        #     writer.add_scalar(
        #         "Accuracy/train per Epoch (smooth)",
